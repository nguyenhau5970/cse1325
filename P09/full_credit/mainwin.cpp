...
Mainwin::Mainwin() {
    ...
    //Transcript
    Gtk::MenuItem *menuitem_new_transcript = Gtk::manage(new Gtk::MenuItem("_New_Transcript", true));
   ...
   // Add a little space 
    ...
    transcript_button->set_tooltip_markup("Create new transcript");
    transcript_button->signal_clicked().connect([this] {this->on_new_transcript_click();});
    toolbar->append(*transcript_button);
    
    //Grade Logo
    Gtk::Image* grade_image = Gtk::manage(new Gtk::Image{"grades.png"});
...
void Mainwin::on_new_transcript_click() {  // Create a new section
        // Select Course
